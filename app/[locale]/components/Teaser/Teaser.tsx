            ...
            <div className={`dark flex flex-row ${position_css}`}>
                <div className='mx-[2.25rem] md:mx-[5.25rem] md:max-w-[45.635%]'>
                    {heading && <div data-id='h2-text'
                        ...
                        {heading}</div>}
                    {content && <div data-id='paragraph-text' className={'font-extralight text-md md:text-xl tracking-[0.063rem] p-0 text-white mt-7.5 drop-shadow-[0_2.5px_2.5px_rgba(0,0,0,0.8)] line-clamp-5 whitespace-break-spaces'}>
                        {content}
                    </div>}
                    {cta?.[0]?.text && ctaLink && <div><Link
                        url={ctaLink}
